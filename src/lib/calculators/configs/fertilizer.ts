...
function createCropFertConfig(slug: string, data: CropData): CalculatorConfig {
  ...
  return {
    ...
    howToUse: `Enter your field size and ${data.name.toLowerCase()} yield goal. The calculator uses crop-specific nutrient uptake rates to determine how much fertilizer for ${data.name.toLowerCase()} per acre — including nitrogen, phosphorus, and potassium. Soil test values are subtracted to show only the supplemental fertilizer required.`,
...
const npk: CalculatorConfig = {
  ...
  howToUse: 'This nutrient calculator starts with your total field size in acres. Then input the recommended rates from your soil test report for nitrogen, phosphorus, and potassium. The fertilizer application calculator multiplies each rate by your acreage to give total pounds needed.',
  whyItMatters: 'Applying the correct NPK rates prevents over-fertilization which wastes money and pollutes waterways, while under-fertilization limits crop yield. This crop fertilizer calculator ensures accurate calculations based on soil tests — the foundation of profitable farm fertilizer management.',
  methodology: 'This ag fertilizer calculator uses a simple multiplication formula: Total nutrient (lbs) = Rate (lbs/acre) × Acres. Rates should come from a certified soil test laboratory. The calculator does not account for nutrient credits from previous crops or organic matter — use the Nitrogen Rate Calculator for that.',
  commonMistakes: [
...
const lime: CalculatorConfig = {
  ...
  howToUse: 'Enter your field size in acres, then input your current soil pH and target pH from your soil test report. This lime application rate calculator uses the buffer pH value to account for your soil\'s resistance to pH change, estimating the tons of lime per acre needed for your entire field.',
  whyItMatters: 'Soil pH controls nutrient availability — most crops perform best between pH 6.0 and 7.0. Without proper pH correction, even perfect fertilizer rates fail because nutrients like phosphorus and molybdenum become locked up in acidic soils, reducing uptake and wasting your fertilizer investment.',
...
const compost: CalculatorConfig = {
  ...
  howToUse: 'This compost estimator helps you figure out how much compost you need. Enter the area in square feet and the desired depth in inches. Optionally adjust the compost density if you know your product weight. The calculator converts your inputs into cubic yards and total pounds so you can order the right amount.',
  whyItMatters: 'Garden compost improves soil structure, water-holding capacity, and microbial activity while slowly releasing nutrients over time. Applying the right amount avoids both under-amendment, which limits benefits, and over-application, which can cause excess salts or nutrient imbalances.',
...
const nitrogen: CalculatorConfig = {
  ...
  howToUse: 'This N rate calculator takes your field size, yield goal, and the nitrogen factor for your crop (pre-filled for corn). Select the previous crop to apply nitrogen credits, and enter your soil organic matter percentage. The calculator subtracts all credits from the gross nitrogen per acre need to show your net application rate.',
  whyItMatters: 'Nitrogen is the most expensive and environmentally sensitive nutrient in crop production. A proper nitrogen needs assessment prevents over-application, which wastes money, increases nitrate leaching into groundwater, and can cause lodging. Under-applying limits yield potential and profit.',
